...
type Biometrics = "Biometrics" | "Face ID" | "Touch ID";
...
export interface ISecurityModel {
  ...
  setSensor: Action<ISecurityModel, Biometrics | null>;
...
export const security: ISecurityModel = {
  initialize: thunk(async (actions) => {
    log.d("Initializing");
    const storedLoginMethods = ((await getItemObject(StorageItem.loginMethods)) ??
      []) as LoginMethods[];
    const loginMethods = new Set<LoginMethods>(storedLoginMethods);

    if (loginMethods.delete(LoginMethods.fingerprint)) {
    ...
    actions.setSensor(null);
    log.d("Done");
  }),

  checkPincode: thunk(async (_, pincodeAttempt) => {
    const pincode = await getPin();
    return pincode === pincodeAttempt;
  }),
  ...
  setFingerprintEnabled: thunk(async (actions, enable, { getState }) => {
    ...
    if (enable) {
      Alert.alert("Biometrics are not supported on Windows yet.");
    }
  }),

  fingerprintStartScan: thunk(async () => {
    Alert.alert("Biometrics are not supported on Windows yet.");
    return false;
  }),
